...
- URL: https://spacecatx.me/international-apple-payment-smishing-lnk-ink/
- Published: 2026-07-27T08:16:00
- Modified: 2026-09-16T08:27:31
...
![국제발신 apple 스미싱 문자들](https://spacecatx.me/wp-content/uploads/2026/07/a437c71f-ca63-49c6-aa5c-02d26bd02468-1024x896.png)
...
![lnk.ink의 정체](https://spacecatx.me/wp-content/uploads/2026/07/image-22.png)
...
그리고 확인해보니 이번 문자들에는 lnk.ink 말고도 **ln.run**이라는 다른 단축 서비스도 같이 쓰이고 있었어요. 결국 특정 서비스 하나의 문제가 아니라, 단축 링크를 만들어주는 서비스는 종류 상관없이 다 이런 식으로 이용당할 수 있다는 거예요.

여기서 더 흥미로운 게 있는데요, 이스트시큐리티 알약M의 스미싱 신고 집계를 보니까 2025년 9월에 이미 “AppleID 이상감지, 곧사용제한예정”이라는 거의 동일한 문구가 신고돼 있었어요. 근데 그때는 lnk.ink가 아니라 **lihi.cc**라는 또 다른 단축 서비스가 쓰였더라구요.
...
참고로 lnk.ink랑 이름이 비슷해서 헷갈릴 수 있는데, 윈도우 바로가기 파일 확장자인 **.lnk 파일**이랑은 완전히 다른 거예요.
...
바로 애플 공식 설정이나 공식 사이트에서 비밀번호부터 바꿔주세요.

```
설정 > 내 이름 > 로그인 및 보안 > 암호 변경
```

비밀번호를 바꿀 때 “다른 기기에서 로그아웃” 옵션이 보이면 꼭 선택하세요. 혹시 누가 이미 로그인해뒀더라도 이걸로 한 번에 끊어낼 수 있어요.

그다음 기기 목록에서 모르는 기기가 있는지 보시고, 낯선 기기는 바로 제거해주세요. 이중 인증에 등록된 전화번호랑 계정 복구 정보, 최근 구입 내역도 같이 확인하시면 돼요.
...
이 경우는 실제로 로그인당했을 가능성이 높아서 제일 급해요. 위 순서대로 비밀번호 변경이랑 다른 기기 로그아웃부터 바로 하시고, 신뢰하는 전화번호에 모르는 번호가 추가돼 있지 않은지 꼭 확인하세요. 비밀번호가 이미 바뀌어서 로그인이 안 된다면 혼자 해결하기 어려우니 애플 고객센터(080-333-4000)로 바로 연락하시는 게 빨라요.

요청하지도 않은 애플 계정 코드 문자가 이후에 계속 온다면 [이 글](https://spacecatx.me/apple-account-code-international-sms/)에서 대처 순서를 확인해보세요.
...
[유튜브 스크립트 무한로딩, 이렇게 해결했어요! (유튜브 스크립트 안뜸)](https://spacecatx.me/youtube-transcript-loading/)
