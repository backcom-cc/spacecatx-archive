# 마이크로소프트 “특이한 로그인 활동” “비정상적인 활동 감지” 뜰 때 확인할 것(2026)

- URL: https://spacecatx.me/microsoft-account-unusual-login-activity/
- Published: 2026-05-23T08:05:00
- Modified: 2026-05-23T11:20:23
...
여기서 직접 로그인한 뒤 보안 메뉴에서 최근 활동을 확인하세요. 실제로 이상한 기록이 있으면 그때부터 조치하면 됩니다.
...
```
account.microsoft.com > 보안 > 최근 활동
```

여기서 “비정상적인 활동이 감지됨” 기록이 보이면 눌러서 국가/지역부터 확인하세요.
...
특히 해외에서 **로그인 완료**로 찍혀있거나, 내가 바꾼 적 없는데 **암호 재설정됨**이 보이면 바로 다음 단계로 넘어가세요.
...
```
account.microsoft.com/security > 고급 보안 옵션 > 2단계 인증
```

문자 인증도 안 하는 것보다는 훨씬 낫지만, 가능하면 Microsoft Authenticator 같은 인증 앱을 쓰는 게 더 안전해요.
...
```
account.microsoft.com/security > 고급 보안 옵션
```

여기서 복구 이메일, 전화번호, 인증 앱이 전부 내 것인지 확인하고, 모르는 게 있으면 바로 삭제하세요.
...
```
outlook.com > 설정(톱니바퀴) > 메일 > 규칙
```

```
outlook.com > 설정(톱니바퀴) > 메일 > 전달
```

비밀번호만 바꾼다고 끝이 아니에요. 메일함 안에 남아있는 자동 전달 설정까지 지워야 진짜로 막을 수 있어요.
...
Microsoft도 특이한 로그인 알림을 받았다면 계정 보안 페이지에서 최근 활동을 확인하라고 안내하고 있습니다. 자세한 내용은 [Microsoft 공식 지원 문서](https://support.microsoft.com/en-us/account-billing/what-happens-if-there-s-an-unusual-sign-in-to-your-account-eba43e04-d348-b914-1e95-fb5052d3d8f0)에서 확인할 수 있어요.
...
[구글에서 “비정상적인 활동이 감지되었습니다” 메일이 왔어요](https://spacecatx.me/google-unusual-activity-detected-email/)

[유튜브 로그인 실패… 혹시 해킹당한 걸까? 지메일은 잘 되는데 안 될 때 확인할 3가지](https://spacecatx.me/youtube-login-error-gmail-ok-fix-3/)
